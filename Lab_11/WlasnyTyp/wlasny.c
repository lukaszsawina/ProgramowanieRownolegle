...
int main(int argc, char** argv) {
    ...
    while (1) {
        if (rank == 0) {
            ...
            if (wyslane.val != 0.0) {
                MPI_Recv(&odebrane, 1, Wlasny_typ, size - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            ...
            MPI_Recv(&odebrane, 1, Wlasny_typ, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            ...
            MPI_Send(&wyslane, 1, Wlasny_typ, 0, 0, MPI_COMM_WORLD);
